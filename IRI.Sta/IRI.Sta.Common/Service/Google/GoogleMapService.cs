...
public static class GoogleMapService
{
    ...
    public async static Task<Response<GooglePlacesResult>> SearchAsync(string query, string key)
    {
        ...
        return await Helpers.NetHelper.HttpGetAsync<GooglePlacesResult>($"{googlePlacesApiUrl}query={query}&key={key}");
    ...
    {
        return Helpers.NetHelper.HttpGet<GooglePlacesResult>($"{googlePlacesApiUrl}query={query}&key={key}");
    }
