...
namespace IRI.Sta.Common.Security
{
    public class SignedJsonWebToken
    {
        ...
        {
            string header = JsonConvert.SerializeObject(this).ToBase64UrlString();
            string claims = JsonConvert.SerializeObject(this.claims).ToBase64UrlString();
        ...
        public static SignedJsonWebToken Parse(string token, string secretKey)
        {
            ...
            var jwt = JsonConvert.DeserializeObject<SignedJsonWebToken>(header);
