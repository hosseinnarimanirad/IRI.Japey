...
namespace IRI.Test.MainTestProject
{
    ...
    public class WebMercatorUtilityTest
    {
        ...
        {
            var rowColumn01 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: 60, geocentricLongitude: -140, zoom: 3);
            Assert.AreEqual(0, rowColumn01.X);
            Assert.AreEqual(2, rowColumn01.Y);

            var rowColumn02 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: -60, geocentricLongitude: -140, zoom: 3);
            Assert.AreEqual(0, rowColumn02.X);
            Assert.AreEqual(5, rowColumn02.Y);

            var rowColumn03 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: -84, geocentricLongitude: 40, zoom: 3);
            Assert.AreEqual(4, rowColumn03.X);
            Assert.AreEqual(7, rowColumn03.Y);

            var rowColumn04 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: 31.1, geocentricLongitude: 42.5, zoom: 8);
            Assert.AreEqual(158, rowColumn04.X);
            Assert.AreEqual(104, rowColumn04.Y);

            var rowColumn05 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: 84.6, geocentricLongitude: -179.3, zoom: 6);
            Assert.AreEqual(0, rowColumn05.X);
            Assert.AreEqual(0, rowColumn05.Y);

            var rowColumn06 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: 4.6, geocentricLongitude: -170.3, zoom: 6);
            Assert.AreEqual(1, rowColumn06.X);
            Assert.AreEqual(31, rowColumn06.Y);

            var rowColumn07 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: -4.3, geocentricLongitude: -170.3, zoom: 6);
            Assert.AreEqual(1, rowColumn07.X);
            Assert.AreEqual(32, rowColumn07.Y);

            var rowColumn08 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: -79.5, geocentricLongitude: -2, zoom: 6);
            Assert.AreEqual(31, rowColumn08.X);
            Assert.AreEqual(56, rowColumn08.Y);

            var rowColumn09 = Ham.SpatialBase.Mapping.WebMercatorUtility.LatLonToImageNumber(geocentricLatitude: 22.5, geocentricLongitude: 2, zoom: 6);
        ...
        {
            var boundingBox01 = Ham.SpatialBase.Mapping.WebMercatorUtility.GetWgs84ImageBoundingBox(6, 0, 3);
            ...
            var boundingBox02 = Ham.SpatialBase.Mapping.WebMercatorUtility.GetWgs84ImageBoundingBox(1, 5, 3);
            ...
            var boundingBox03 = Ham.SpatialBase.Mapping.WebMercatorUtility.GetWgs84ImageBoundingBox(100, 164, 8);
            ...
            var geocentricBL = MapProjects.GeodeticToMercator(new Ham.SpatialBase.Point(boundingBox03.XMin, boundingBox03.YMin), Ellipsoids.Sphere);
